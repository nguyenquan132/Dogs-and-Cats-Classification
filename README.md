...
The FastAPI-based web application is deployed on Hugging Face Spaces and can be accessed through the following link: 👉 [Live Demo](https://quan12012-cat-dog-classification.hf.space)

The application allows users to upload an image and returns the predicted class (Cat or Dog).
...
```
## 🖥️ Web Interface
The web interface enables users to upload an image and obtain the classification result generated by the model. The uploaded image is displayed alongside the prediction to facilitate visual verification.
